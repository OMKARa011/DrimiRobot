...
import Drimi.modules.sql.feds_sql as sql
from Drimi import (
    DRAGONS,
    EVENT_LOGS,
    LOGGER,
...
from Drimi.modules.helper_funcs.decorators import Drimicallback
from Drimi.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Drimi.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Drimi_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Drimi_fbanned_users.json",
        ...
        if args[0] == "csv":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Drimi_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Drimi_fbanned_users.csv",
...
@Drimicallback(pattern=r"fed_help_")
