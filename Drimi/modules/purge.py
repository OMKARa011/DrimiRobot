...
from Drimi import LOGGER, Zadkiel


@Zadkiel.on_cmd("purge")
@Zadkiel.adminsOnly(permissions="can_delete_messages", is_both=True)
async def purge(c: Zadkiel, m: Message):
...
@Zadkiel.on_cmd("spurge")
@Zadkiel.adminsOnly(permissions="can_delete_messages", is_both=True)
async def spurge(c: Zadkiel, m: Message):
    if m.chat.type != ChatType.SUPERGROUP:
        await m.reply_text(text="ᴄᴀɴɴᴏᴛ ᴘᴜʀɢᴇ ᴍᴇssᴀɢᴇs ɪɴ ᴀ ʙᴀsɪᴄ ɢʀᴏᴜᴘ")
        return
...
@Zadkiel.on_cmd("del")
@Zadkiel.adminsOnly(permissions="can_delete_messages", is_both=True)
async def del_msg(c: Zadkiel, m: Message):
    if m.reply_to_message:
        await m.delete()
        await c.delete_messages(
            chat_id=m.chat.id,
