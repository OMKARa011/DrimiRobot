...
from Drimi import BACKUP_PASS, DB_URL, DEV_USERS, LOGGER, OWNER_ID, dispatcher
from Drimi.modules.helper_funcs.decorators import Drimicmd


@Drimicmd(command="backupdb", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def backup_now(_: Update, ctx: CallbackContext):
    cronjob.run(dispatcher=dispatcher)


@Drimicmd(command="stopjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def stop_jobs(update: Update, _: CallbackContext):
    print(j.stop())
    update.effective_message.reply_text("Scheduler has been shut down")


@Drimicmd(command="startjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
...
from Drimi import updater as u
