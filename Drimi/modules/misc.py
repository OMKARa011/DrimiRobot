...
from Drimi import OWNER_ID, dispatcher
from Drimi.modules.disable import DisableAbleCommandHandler
from Drimi.modules.helper_funcs.alternate import send_action, typing_action
from Drimi.modules.helper_funcs.chat_status import user_admin
from Drimi.modules.helper_funcs.decorators import Drimicmd
...
def src(update, _):
    update.effective_message.reply_text(
        "Hey there! You can find what makes me click [here](https://github.com/Moca96/DrimiRobot).",
        parse_mode=ParseMode.MARKDOWN,
        disable_web_page_preview=True,
    )
...
def wiki(update, context):
    ...
    if len(str(Shinano[1])) == 0:
        ...
        try:
            Drimi = update.effective_message.reply_text(
