...
from Drimi import Zadkiel, dispatcher
from Drimi import telethn as bot
from Drimi.modules.connection import connected
from Drimi.modules.disable import DisableAbleCommandHandler
from Drimi.modules.helper_funcs.alternate import typing_action
from Drimi.modules.helper_funcs.chat_status import (
    ADMIN_CACHE,
    bot_admin,
    can_pin,
    can_promote,
...
from Drimi.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
"""        
@Zadkiel.on_message(filters.command(["staff", "admins", "adminlist"]) & filters.group)
...
@Zadkiel.on_message(filters.command(["adminlist", "staff", "admins"]))
async def admins(client, message):
    try:
        adminList = []
        ownerList = []
        async for admin in Zadkiel.get_chat_members(
        ...
        if len(adminList) == 0:
            ...
            await Zadkiel.send_message(message.chat.id, text2)
        else:
...
@Zadkiel.on_message(filters.command("bots"))
async def bots(client, message):
    try:
        botList = []
        async for bot in Zadkiel.get_chat_members(
        ...
        while len(botList) > 1:
            ...
            await Zadkiel.send_message(message.chat.id, text3)
    except FloodWait as e:
        await asyncio.sleep(e.value)
