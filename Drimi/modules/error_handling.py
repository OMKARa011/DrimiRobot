...
from Drimi import DEV_USERS
from Drimi import LOG_GROUP_ID as ERROR_LOGS
    ...
    if context.error not in errors:
        ...
        if not response:
            ...
            context.bot.send_document(
                ERROR_LOGS,
                open("error.txt", "rb"),
                caption=f"#{context.error.identifier}\n<b>ʏᴏᴜʀ ᴄᴜᴛᴇ 𝑫𝒓𝒊𝒎𝒊 ʜᴀᴠᴇ ᴀɴ ᴇʀʀᴏʀ ғᴏʀ ʏᴏᴜ:"
                f"</b>\n<code>{e}</code>",
                parse_mode="html",
            )
            return
        ...
        context.bot.send_message(
            ERROR_LOGS,
            text=f"#{context.error.identifier}\n<b>Your Cute Drimi Nagisa Have An Error For You:"
            f"</b>\n<code>{e}</code>",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("sᴇxʏ 𝑫𝒓𝒊𝒎𝒊 ᴇʀʀᴏʀ ʟᴏɢs", url=url)]],
            ),
            parse_mode=ParseMode.HTML,
        )


@Drimicmd(command="errors")
