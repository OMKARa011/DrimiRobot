...
from Drimi import DEV_USERS, OWNER_ID, dispatcher, telethn, updater
from Drimi.modules.helper_funcs.alternate import typing_action
from Drimi.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not Drimi.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        Drimi.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Drimi.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        update.effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(Drimi_leave_bt),
        )
