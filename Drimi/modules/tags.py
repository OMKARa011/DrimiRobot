...
from Drimi import telethn as zadkiel
...
from Drimi import Zadkiel as AsuX
...
@zadkiel.on(events.NewMessage(pattern="^/tagall ?(.*)"))
@zadkiel.on(events.NewMessage(pattern="^@all ?(.*)"))
async def mentionall(event):
    ...
    is_admin = False
    try:
        partici_ = await zadkiel(GetParticipantRequest(event.chat_id, event.sender_id))
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in zadkiel.iter_participants(chat_id):
        ...
        if usrnum == 5:
            if mode == "text_on_cmd":
                txt = f"{msg}\n{usrtxt}"
                await zadkiel.send_message(chat_id, txt)
...
@zadkiel.on(events.NewMessage(pattern="^/cancel$"))
