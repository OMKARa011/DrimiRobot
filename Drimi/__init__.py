...
def get_user_list(__init__, key):
    with open("{}/Drimi/{}".format(os.getcwd(), __init__), "r") as json_file:
        return json.load(json_file)[key]


# enable logging
FORMAT = "[𝑫𝒓𝒊𝒎𝒊] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger("[𝑫𝒓𝒊𝒎𝒊]")
LOGGER.info("𝑫𝒓𝒊𝒎𝒊 ɪs sᴛᴀʀᴛɪɴɢ. | ᴀɴ ᴢᴀᴅᴋɪᴇʟ ᴘʀᴏᴊᴇᴄᴛ ᴘᴀʀᴛs. | ʟɪᴄᴇɴsᴇᴅ ᴜɴᴅᴇʀ ɢᴘʟᴠ3.")
...
SPAMWATCH_SUPPORT_CHAT = "@Chatting_hub51"
...
telegraph = Telegraph()
telegraph.create_account(short_name="Drimi")
    ...
    if not isinstance(entity, Chat):
        ...
        try:
            ...
            for Zadkiel in apps:
                if Zadkiel != client:
                    try:
                        entity = await Zadkiel.get_chat(entity)
                    except (PeerIdInvalid, ChannelInvalid):
                        pass
                    else:
                        entity_client = Zadkiel
                        break
            else:
                entity = await Zadkiel.get_chat(entity)
                entity_client = Zadkiel
...
from Drimi.modules.helper_funcs.handlers import CustomMessageHandler, CustomRegexHandler

tg.RegexHandler = CustomRegexHandler
tg.MessageHandler = CustomMessageHandler

from Drimi.modules.helper_funcs.handlers import CustomCommandHandler
...
try:
    from Drimi.antispam import antispam_cek_user, antispam_restrict_user, detect_user
